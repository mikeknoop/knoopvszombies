<?php if (isset($_GET['action']) && ($_GET['action'] == 'edit' || $_GET['action'] == 'save') && isset($_GET['target']) && $_GET['target'] != ''): ?>
  ...
  <?php
    ...
    if ($_GET['action'] == 'save')
    {
              ...
              {           
                ...
                switch ($value)
                {
                  case "admin":
                    ...
                    $sql = "UPDATE ".FORUM_DATABASE.".GDN_User SET admin = 1 WHERE UserID = '{$user['uid']}'";
                    if (!$GLOBALS['Db']->Execute($sql))
                    {
                      throw new Exception('Error updating user.');
                    }
                  ...
                  case "moderator":
                    ...
                    $sql = "UPDATE ".FORUM_DATABASE.".GDN_User SET admin = 0 WHERE UserID = '{$user['uid']}'";
                    if (!$GLOBALS['Db']->Execute($sql))
                    {
                      throw new Exception('Error updating user.');
                    }
      ...
      if (isset($_POST['notify'])  && (count($saved) > 0 || isset($email_changes['Removed From Game'])))
      {
        // Mail user at email address
        $to = $user['email'];
