<?php
  ...
  if (isset($_GET['action']) && $_GET['action'] == 'toggle_fb'){
                 $GLOBALS['User']->UpdateForumUserColumn($_SESSION['uid'], 'Photo', 'https://graph.facebook.com/'.$_SESSION['fb_id'].'/picture');
                 $GLOBALS['User']->UpdateUserColumn($_SESSION['uid'], 'fb_image', 1);
                 header('Location: //'.DOMAIN.'/changepicture'); 
  }
  ...
  if (isset($_GET['action']) && $_GET['action'] == 'submit')
  {
		...
		$GLOBALS['User']->UpdateForumUserColumn($_SESSION['uid'], 'Photo', 'http://'.DOMAIN.'/img/user/u'.$_SESSION['uid'].'.jpg');
	        $GLOBALS['User']->UpdateUserColumn($_SESSION['uid'], 'fb_image', '0');
      ...
      <div id="content">
        ...
        <div id="body_content">
          ...
          <div id="squad_container">
            ...
            <?php if (!$_SESSION['using_fb'] || 1): ?>
            <div class="squad_header">
              <p>Upload a photo of for use on the site.
            </div>
            ...
            <?php if($_SESSION['using_fb'] && !$_SESSION['fb_image']): ?>
            <div class="squad_form"><div class="squad_row">
            <form id="squad" name="changepicture" action="//osundead.com/changepicture/toggle_fb" method="POST" enctype="multipart/form-data">
	      <input type="submit" value="Use Facebook Picture" class="squad_form_submit" />
            </form>
            </div></div>
            <?php endif; ?>
            </div>
