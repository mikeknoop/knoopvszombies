    ...
    switch ($_GET['step'])
    {
      ...
      case 5:
          ...
          if ($_GET['state'] == 'auth')
          {
            ...
            if (isset($_GET['code']))
            {
                try
                {
                  ...
                  $uri = 'https://graph.facebook.com/oauth/access_token?client_id='.FB_APP_ID.'&redirect_uri='.urlencode('http://'.DOMAIN.'/oauth/init').'&client_secret='.FB_SECRET.'&code='.$code;
                  $json_decode = false;               
        ...
        else
        {
          ...
          if ($_SESSION['approved'] && $GLOBALS['User']->AccountComplete($_SESSION['uid']))
          { 
            header('Location: //'.DOMAIN.'/signup/6');
            exit;
          }
